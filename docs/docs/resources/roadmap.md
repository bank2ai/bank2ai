...
- **More transfer rails.** `prepare-transfer` ships with `domestic-IS`, `sepa`, `sepa-instant`, and `swift` in the `Rail` enum. Adding more (Faster Payments, ACH, FedNow, Pix, UPI, Interac, …) is additive and continues without breaking existing clients.

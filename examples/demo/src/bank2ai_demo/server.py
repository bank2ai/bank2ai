...
_MINIMAL_SUPPRESSED_FIELDS = frozenset({"counterparty", "properties"})


def _apply_verbosity(t: Transaction, verbosity: str) -> Transaction:
    """At `minimal`, drop the heavy `counterparty` and `properties`
    objects; everything else passes through. Unset optional keys are
    elided from the wire payload, so the suppressed objects are simply
    absent.
    """

    if verbosity == "full":
        return t
    for field in _MINIMAL_SUPPRESSED_FIELDS:
    ...
    if search_text:
        ...
        def _matches(t: dict) -> bool:
            candidates: list[str] = [
                ...
                t.get("originalDescription") or "",
            ]
            counterparty = t.get("counterparty") or {}
            candidates.append(counterparty.get("name") or "")
            candidates.extend(
                str(v) for v in (t.get("properties") or {}).values() if v
            )

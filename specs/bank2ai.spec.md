...
* **`Transaction`**. *Profile of:* ISO 20022 `EntryDetails2`, Berlin Group PSD2 `transactions`, and Open Finance `cardTransactions`. One shape covers both account and card transactions. Always present: id, accountId, description, amount (in the user's default currency, negative = expense), bookingDate (ISO 8601). Optional lean fields populated when known: status, counterpartyName, categoryId, originalCurrency / originalAmount (FX original), transactionDate (when the card was swiped, distinct from posting), maskedPan (which card was used), merchantCategoryCode (ISO 18245), and a typed counterparty whose `postalAddress` carries the Open Finance `cardAcceptorAddress` (`townName`, `country`, `postCode`, `streetName`, `buildingNumber`, `countrySubDivision`). Audit / reconciliation fields populated for the `full` verbosity and from `get-transaction`: valueDate, categoryRaw, transactionCode (ISO 20022 BankTransactionCode), proprietaryBankTransactionCode, remittanceInformation, endToEndId, mandateId, creditorId, purposeCode, entryReference, additionalInformation. Servers drop unset optional fields on the wire so every row stays compact; clients MUST tolerate any subset.

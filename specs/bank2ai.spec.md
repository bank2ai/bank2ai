...
* **`Transaction`**. *Profile of:* ISO 20022 `EntryDetails2`, Berlin Group PSD2 `transactions`, and Open Finance `cardTransactions`. One shape covers both account and card transactions. Always present: id, accountId, description, amount (in the user's default currency, negative = expense), date (ISO 8601 — the booking date when `status` is `Booked`, the authorisation / point-of-sale date when `status` is `Pending`). Profiles ISO 20022 / Berlin Group `bookingDate` but is renamed to `date` because pending authorisations have no booking date yet — keeping a single, always-populated date field avoids forcing servers to fabricate one and lets clients sort and chart by `date` without branching on `status`. Optional lean fields populated when known: status, counterpartyName, categoryId, originalCurrency / originalAmount (FX original), transactionDate (card swipe / authorisation date for `Booked` card entries where it differs from the posting `date` — omitted on `Pending`), maskedPan (which card was used), merchantCategoryCode (ISO 18245), and a typed counterparty whose `postalAddress` carries the Open Finance `cardAcceptorAddress` (`townName`, `country`, `postCode`, `streetName`, `buildingNumber`, `countrySubDivision`). Audit / reconciliation fields populated for the `full` verbosity and from `get-transaction`: valueDate, categoryRaw, transactionCode (ISO 20022 BankTransactionCode), proprietaryBankTransactionCode, remittanceInformation, endToEndId, mandateId, creditorId, purposeCode, entryReference, additionalInformation. Servers drop unset optional fields on the wire so every row stays compact; clients MUST tolerate any subset.

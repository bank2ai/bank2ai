...
* **`Transaction`**. *Profile of:* ISO 20022 `EntryDetails2`, Berlin Group PSD2 `transactions`, and Open Finance `cardTransactions`. One shape covers both account and card transactions. Always present: id, accountId, description, amount (in the user's default currency, negative = expense), date (ISO 8601 — the booking date when `status` is `Booked`, the authorisation / point-of-sale date when `status` is `Pending`). Profiles ISO 20022 / Berlin Group `bookingDate` but is renamed to `date` because pending authorisations have no booking date yet — keeping a single, always-populated date field avoids forcing servers to fabricate one and lets clients sort and chart by `date` without branching on `status`. The `get-transactions` `verbosity` parameter selects between two payload shapes: at `minimal` (default), each Transaction additionally carries `categoryId` and `originalCurrency` / `originalAmount` for FX entries — the fields an LLM typically needs to answer everyday questions, with the merchant / counterparty name read off `description`. At `full` (and from `get-transaction`), Transactions additionally carry every optional ISO 20022 / Open Finance audit field the server can populate. Servers drop unset optional fields on the wire so every row stays compact; clients MUST tolerate any subset.
